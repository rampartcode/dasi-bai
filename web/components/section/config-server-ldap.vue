...
const { isSubmitting, defineField, handleSubmit, handleReset, errors } =
  useForm({
    validationSchema: yup.object({
      ...
      usernameAttribute: yup
        .string()
        .required("usernameAttribute é obrigatório"),
    }),
  });
...
<template>
  <transition name="scroll-x">
    <div
      v-show="tabSelected === 'server-ldap'"
      ...
      <div class="max-w-lg h-[90%] mx-auto px-1 overflow-y-scroll">
        <form @submit.prevent="onSubmit" class="space-y-4">
          ...
          <div class="flex gap-4 justify-center mt-4">
            <button
              type="submit"
              class="bg-brand border !border-brand px-5 py-2.5 text-white text-nowrap font-medium rounded-md transition-all hover:bg-brand-hover outline-none focus:outline-none"
            >
            ...
            <button
              type="button"
              :disabled="ldapSettingsIsEmpty"
              class="bg-white/10 border px-5 py-2.5 text-white text-nowrap font-medium rounded-md transition-all hover:bg-white hover:!text-neutral-800 outline-none focus:outline-none disabled:opacity-40 disabled:pointer-events-none"
            >
              Testar Conexão
            </button>
          </div>
